...
void compute_offsets(float *_offset){
	...
	float curr_time = fmodf(elapsed_time, loop_duration);

	_offset[0] = cosf(curr_time * scale) * 0.5f;
	_offset[1] = sinf(curr_time * scale) * 0.5f;
}
...
void display(){
	...
	glUniform2f(offset_location, offsets[0], offsets[1]);
	glBindBuffer(GL_ARRAY_BUFFER, position_buffer_object);
